class User < ApplicationRecord
  ...
  def purchase_cart_products!
    order = self.order.create(user: self, order_date: Time.now)
    get_cart_products_with_qty.each do |product, qty|
      order.line_item.create(order_id: order.id, product_id: product.id, quantity: qty, unit_price: product.price)
    end
